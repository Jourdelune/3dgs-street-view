...
def generate_planar_projections_from_equirectangular(
    ...
    if samples_per_im == 8:
        ...
        logging.error(
            f"Unsupported samples_per_im: {samples_per_im}. Only 8 or 14 are supported."
        )
        sys.exit(1)
    ...
    num_ims = len(image_files)

    logging.info(f"Processing {num_ims} images from {image_dir}")

    for idx, i in enumerate(image_files):
        logging.info(f"Processing image {idx + 1}/{num_ims}: {i}")
        ...
        im = torch.tensor(np.array(pil_image), dtype=torch.float32, device=device)
        ...
        for u_deg, v_deg in yaw_pitch_pairs:
            omnicv_pers_tensor = (
                equirect2persp(
                    im, fov, u_deg, v_deg, planar_image_size[1], planar_image_size[0]
                )
                * 255.0
            )
            pers_image_np = (
            ...
            # Convert numpy array to PIL Image and save with metadata
